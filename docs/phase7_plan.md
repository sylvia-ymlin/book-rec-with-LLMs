...
| | **Content Match**: `is_favorite_author` (作者命中), `is_favorite_category` (类别命中) | 2 |
| | **Time Diff**: 出版时间 vs 用户平均阅读年代的差值 | 1 |
| | **Complexity Match**: 书籍描述长度 (Proxy for complexity) 与 用户历史均值的差值 | 1 |
...
### 实验记录 (Feature Engineering)

#### 1. Complexity Match (Desc Length)
*   **特征**: `len_diff` (用户平均描述长度 vs 书籍描述长度)
*   **结果**: Feature Importance 极低 (0.0009)，MRR 下降。
*   **结论**: 描述长度不是区分度高的特征。

#### 2. Author Match (Avg Rating)
*   **特征**: `u_auth_avg` (用户对作者的历史均分), `u_auth_match` (是否读过)
*   **结果**:
    *   Feature Importance: `u_auth_match` (0.0049), `u_auth_avg` (0.0018)
...
- [x] 更新 TECHNICAL_REPORT.md (Next Step)
- [ ] Git commit & push
