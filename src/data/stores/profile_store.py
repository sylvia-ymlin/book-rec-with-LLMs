"""
User profile and favorites store.

Moved from `src/user/profile_store.py` into `src.data.stores`.
"""
...
def _ensure_store_file() -> None:
    try:
        ...
        logger.error("Failed to ensure profile store: %s", e)
        raise
...
def get_reading_stats(user_id: str) -> Dict[str, int]:
    ...
    for _isbn, meta in favorites.items():
