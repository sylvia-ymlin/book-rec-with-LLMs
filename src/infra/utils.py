...
def enrich_book_metadata(meta: dict | None, isbn: str) -> dict:
    ...
    # Normalize legacy field names (some records store cover in "image")
    if "thumbnail" not in meta and meta.get("image"):
        meta["thumbnail"] = meta.get("image")
    ...
    author_norm = str(author).strip().lower() if author is not None else "unknown"
    is_unknown_author = author_norm in {"", "unknown", "none", "null", "nan"}

    # 2. Validation Check
    is_valid_thumb = thumbnail and str(thumbnail).lower() not in [
        "nan",
        "none",
        "",
        "null",
    ] and "/assets/cover-not-found.jpg" not in str(thumbnail) and "cover-not-found" not in str(thumbnail)

    # 3. Fetch if needed
    should_fetch_remote = (not title) or (not is_valid_thumb) or is_unknown_author
    if should_fetch_remote:
        # Lazy import to avoid circular dependency
        from src.infra.cover_fetcher import fetch_book_cover

        fetched_cover, fetched_authors, fetched_desc = fetch_book_cover(str(isbn), str(title or ""))
        ...
        if is_unknown_author and fetched_authors != "Unknown":
            meta["authors"] = fetched_authors

        if (not description or str(description).strip().lower() in {"", "none", "null", "nan"}) and fetched_desc:
            meta["description"] = fetched_desc

    # 4. Final Fallback
    final_thumb = meta.get("thumbnail")
    if not final_thumb or str(final_thumb).lower() in [
        "nan",
        "none",
        "",
        "null",
    ] or "cover-not-found" in str(final_thumb):
