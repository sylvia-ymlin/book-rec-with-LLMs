...
class LLMFactory:
    ...
    @staticmethod
    def create(
        provider: Literal["openai", "ollama", "mock", "groq", "deepseek"] = "openai",
        ...
        if provider == "deepseek":
            ...
            if not api_key:
                raise ValueError(
                    "DeepSeek API Key is required for 'deepseek' provider."
                )

            # DeepSeek is OpenAI-compatible; use ChatOpenAI with DeepSeek base URL.
            return ChatOpenAI(
                api_key=SecretStr(api_key),
