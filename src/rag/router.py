...
from src.infra.config import FRESHNESS_THRESHOLD, HYBRID_DEFAULT_ALPHA
...
class QueryRouter:
    ...
    def _detect_freshness(
        ...
        from src.infra.config import (
            ROUTER_FRESHNESS_KEYWORDS,
            ROUTER_STRONG_FRESHNESS_KEYWORDS,
        )
        ...
        freshness_fallback = bool(
            lower_words & ROUTER_STRONG_FRESHNESS_KEYWORDS
        )
    ...
    def _route_by_rules(
        ...
        """
        from src.infra.config import ROUTER_DETAIL_KEYWORDS, ROUTER_NL_KEYWORDS
        ...
        if any(w.lower() in ROUTER_NL_KEYWORDS for w in words):
            logger.info(
                "Router (rules): NL keywords -> DEEP (Temporal=%s, Freshness=%s)",
                is_temporal,
                freshness_fallback,
            )
            return {
        ...
        if len(words) <= 6:
            logger.info(
                "Router (rules): Keyword/Title -> FAST (Temporal=%s, Freshness=%s)",
                is_temporal,
                freshness_fallback,
            )
            return {
        ...
        logger.info(
            "Router (rules): Long query -> DEEP (Temporal=%s, Freshness=%s)",
            is_temporal,
            freshness_fallback,
        )
        return {
    ...
    def route(self, query: str) -> Dict[str, Any]:
        ...
        if clf is not None:
            try:
                ...
                logger.warning(
                    "Intent classifier failed, falling back to rules: %s", e
                )

        return self._route_by_rules(
            cleaned_query, words, is_temporal, freshness_fallback, target_year
        )
